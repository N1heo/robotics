...
const int maxSpeed = 140; //max PWM value written to motor speed pin. It is typically 255.
const int minSpeed = 90; //min PWM value at which motor moves
float angle; //due to how I orientated my MPU6050 on my car, angle = roll
float targetAngle = 0;
int equilibriumSpeed = 136; //rough estimate of PWM at the speed pin of the stronger motor, while driving straight 
...
void backwardDriving (){//called by void loop(), which isDriving = true
  int deltaAngle = round(targetAngle - angle); //rounding is neccessary, since you never get exact values in reality
  backward();
  if (deltaAngle != 0){
    controlBackSpeed ();
...
void controlBackSpeed (){//this function is called by driving ()
  int deltaAngle = round(targetAngle - angle);
  int targetGyroX;
